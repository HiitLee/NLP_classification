...
logger = logging.getLogger('elmoforkorean')
logger.setLevel(logging.INFO)

filehandler = logging.FileHandler("elmoforkorea.log")
...
def train_model(epoch, opt, model, optimizer,
  ...
  for w, c, lens, masks in zip(train_w, train_c, train_lens, train_masks):
    ...
    if cnt % opt.eval_steps == 0 or cnt % len(train_w) == 0:
      if valid is None:
        train_ppl = np.exp(total_loss.cpu() / total_tag)
...
def train():
  ...
  print("token_embedder_name:", token_embedder_name)

  token_embedder_max_chars = config['token_embedder'].get('max_characters_per_token', None)
  
  print('token_embedder_max_chars:', token_embedder_max_chars)
  ...
  print('opt.valida_patch:', opt.valid_path)
