...
class LSTM_Maxpool(nn.Module):
    ...
    def __init__(self, args, weight=None):
        super(LSTM_Maxpool, self).__init__()
        self.args = args
        
        if(weight==None):
            self.embed = nn.Embedding(args.vocab,args.LSTM_Maxpool_embed)
        else:
            self.embed = nn.Embedding.from_pretrained(weight).cuda()


        self.bilstm = nn.LSTM(args.LSTM_Maxpool_embed, args.LSTM_Maxpool_hidden, num_layers=args.LSTM_Maxpool_layer, bidirectional=True)
        self.hidden2label1 = nn.Linear(args.LSTM_Maxpool_embed*2//4, args.num_class)
    ...
    def forward(self, input,seq_lengths):
        ...
        out1 = self.hidden2label1(pooled)
        return self.m(out1)
